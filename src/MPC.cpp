 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int t = 1; t < N; t++) {
      ...
      AD<double> psides0 = CppAD::atan(MPC::polyPrimEval(coeffs, x0));
...
AD<double> MPC::polyeval(Eigen::VectorXd coeffs, AD<double> x)
{
...
AD<double> MPC::polyPrimEval(Eigen::VectorXd coeffs, AD<double> x)
{
  AD<double> result = 0.0;
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs)
{
  ...
  for (unsigned int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -1*std::numeric_limits<double>::max();
  ...
  for (unsigned int t = 0; t < N - 1; t++)
  {
    result.push_back(solution.x[delta_start + t]);
  }
  for (unsigned int t = 0; t < N - 1; t++)
  {
    result.push_back(solution.x[a_start + t]);
  }
  for (unsigned int t = 0; t < N - 1; t++)
  {
